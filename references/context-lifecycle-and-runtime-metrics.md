...
The largest measurement trap is mixing different work. A token reduction is not meaningful when the pilot contains smaller tasks, a cheaper model route, fewer tool results, or fewer real decisions. Exclude `not_achieved` epochs because failures often stop early and consume fewer tokens; counting those turns would reward failure and bias the experiment toward an apparently cheaper policy. Choose a stable cohort before collecting data and do not relabel epochs after seeing the report.
