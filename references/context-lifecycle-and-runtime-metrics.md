...
For a Git-backed fresh role, resolve the selected Worktree's `--absolute-git-dir` and `--git-common-dir` immediately before creation, validate that they belong to the intended repository, deduplicate them, and add only the necessary roots to `workspace-write`. Keep the project-selected approval reviewer, network policy, and ordinary workspace root; do not substitute `danger-full-access` or broaden access to another repository or user directory. Before using a new capability surface for a measured role, run one disposable, content-free probe that (1) creates and releases required locks, (2) performs a local fast-forward equivalent covering the branch ref, `ORIG_HEAD`, and index, (3) proves an out-of-scope harmless write is denied without controller approval, and (4) leaves the real project branch, Worktree, existing refs, and pending operations unchanged after cleanup. A capability-profile correction creates a new cohort: keep prior completed epochs immutable and historically visible, but do not pool their token samples or outcomes with the corrected cohort.
...
Codex Desktop may own an isolated stdio App Server connection that cannot be tapped by an external observer. In that case, create a blank non-ephemeral thread through a controlled App Server connection, arm `scripts/desktop_rollout_usage_adapter.py` before the first turn, and then resume the same persisted thread in Desktop. The arm operation must confirm that no rollout exists yet. Collection may replay the resulting local rollout only for that pre-armed epoch; it validates every completed turn's exact model and reasoning route, hashes protocol identities in memory, records compact generations without their replacement history, and persists only the same terminal counters accepted from the live App Server adapter. Existing or active unarmed threads are ineligible. Any format drift, route mismatch, malformed record, or attempt to close an active turn fails closed. This compatibility bridge changes no Codex configuration and is not a general transcript collector.
